...
**Problem.** A test suite that grows past ~30 minutes is run only as a merge gate, never during inner-loop iteration. This means bugs surface days late. But a full run on every push is wasteful when only one spec is relevant to the diff.
...
**Anti-patterns to avoid.** Don't add `await page.waitForTimeout(500)` "to let the other spec finish": that's not synchronization, it's wishful thinking. Don't reset the DB between specs by re-running the seed. It adds tens of seconds per spec; use isolated entities instead.
