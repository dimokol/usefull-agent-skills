...
**This skill is thin, and its value is conditional.** It only does useful work if you have a task board with an API or CLI you can query and update programmatically ({task-board} below, e.g. Jira, Linear, an internal portal, GitHub Projects, whatever tracks your work items). If you don't have one, or your board has no API, skip this skill; there's nothing to reconcile against.
