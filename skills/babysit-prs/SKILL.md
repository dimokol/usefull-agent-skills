...
d) **Post as ONE top-level PR comment:**
   ...
   ```
   If nothing worth flagging after a genuine read, post `"## Self-review\n\nReviewed the diff against CLAUDE.md. No findings."` as the audit trail. Never silently skip the posting.
...
By now CI (Step 2), e2e (Step 3), and self-review (Step 1, if `{doubleReview}` was set) have run, so the reviewer ({reviewer}) has had several minutes to review (whether auto-triggered or asked in Step 4 of the parent). Collect the review, merge with your self-review, apply what's valid.
